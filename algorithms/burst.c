			...
			if (strcmp(bin,curr_word) != 0) {
				...
				for (j=0; j < block_count - 1; ++j) {
					...
					for (k = f_levels - 2; k >= 0; --k) {
                        fprintf(stderr,"Hello! k: %d, f_levels: %d\n", k, f_levels);
                        fprintf(stderr,"total_power[k]: %d\n", cell[j].total_power[k]); 
                        fprintf(stderr,"POWER_THRESH: %d\n", POWER_THRESH);
                        fprintf(stderr,"cell[j].candidate[k]: %d\n", cell[j].candidate[k]);
                        fprintf(stderr,"POWER_THRESH: %d\n", POWER_THRESH);
						...
						if ( (cell[j].candidate[k]) &&
						 ...
						 (!SHARP_OPT || k != cell[j].min_rate_class) ) {
                            fprintf(stderr, " reached calculation block"); 
